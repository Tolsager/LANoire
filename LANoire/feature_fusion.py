...
class CAF(nn.Module):
    ...
    def __init__(self, n_features: int = 2, attention_dim: int = 32):
        ...
        self.CA1 = CA_SA(dim=attention_dim)
        self.CA2 = CA_SA(dim=attention_dim)
        if n_features == 3:
            self.CA3 = CA_SA(dim=attention_dim)
    ...
    def forward(self, *features):
        ...
        feat_z = self.CA1(F_U, features[0].view(b, -1, self.attention_dim))
        feat_z += self.CA2(F_U, features[1].view(b, -1, self.attention_dim))
        if self.n_features == 3:
            feat_z += self.CA3(F_U, features[1].view(b, -1, self.attention_dim))
